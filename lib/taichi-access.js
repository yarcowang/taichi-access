...
(function() {
	 ...
	 * @param object user object, must have two fields: id(defined by this._id), roles. user role "guest", "user", "admin" are reserved
	 * @param object resource object, must have two fields: type, owner. resource type "users", "system" are reserved
	 * @return boolean
	 */
	this.checkUser = function(permission, user, resource) {
		var r = this.id;
		resource.type = resource.type ? resource.type : 'system';
		...
		if ((resource.owner && resource.owner[r] === user[r]) || (resource.type === 'users' && resource[r] === user[r])) {
			return true;
		}
